...
/**
 * Represents a single "droplet", i.e. a bus arriving onto a station.
 */
class Droplet {
...
/**
 * Represents a station popup the user generated by clicking on the canvas.
 */
class StationPopup {
...
/**
 * Select a new data file (station snapshot).
 * Throws an error if `snapshotIndex` is out of bounds.
 *
 * @param snapshotIndex Index of the snapshot station.
 */
...
/**
 * Toggle the pause on the simulation.
 */
...
/**
 * Load the previous available day (previous data file).
 */
...
/**
 * Load the next available day (next data file).
 */
...
/**
 * Resets the current simulation time back to the starting time.
 */
function resetDay() {
...
/**
 * Toggles the simulation speed between `realTimeSecondsPerSimulatedMinute`
 * and `fastForwardedRealTimeSecondsPerSimulatedMinute`.
 */
...
/**
 * Handle a user clicking on the canvas.
 *
 * This will set up or clear station popups if the user clicked near a station.
 */
...
/**
 * Handle the keyboard input on the body element.
 */
...
/**
 * Converts a geographical position on the map into a canvas pixel position.
 *
 * @param location Leaflet LatLng object.
 * @param mapXOffset Map container page X offset.
 * @param mapYOffset Map container page Y offset.
...
/**
 * Converts a screen position into the geographical position on the map.
 *
 * @param pixel Screen pixel position.
 * @param mapXOffset Map container page X offset.
 * @param mapYOffset Map container page Y offset.
 * @param mapPixelOrigin Leaflet map pixel origin.
 */
...
/**
 * "Tick" the droplets.
 * This generates any new droplets that have appeared
 * since the last call and removes any finished droplets.
 *
...
/**
 * Get the DOM element from its ID or throw an error if not found.
 *
...
/**
 * If the loading page is visible, this sets the title content.
 *
...
/**
 * If the loading page is visible, this sets the subtitle content.
 *
...
/**
 * Hides the loading page, making the map and controls visible.
 */
...
/**
 * Draw all the bus stations onto the canvas.
 * Not gated behind the UI option, check the user option before calling.
 *
 * @param p p5 instance.
 * @param mapXOffset Map container page X offset.
 * @param mapYOffset Map container page Y offset.
...
/**
 * Draw the current droplets based on the `activeDroplets` state onto the canvas.
 * Not gated behind the UI option, check the user option before calling.
 *
 * @param p p5 instance.
 * @param currentSimulationTime The current time of day of the simulation.
 * @param mapXOffset Map container page X offset.
 * @param mapYOffset Map container page Y offset.
 * @param mapPixelOrigin Leaflet map pixel origin.
 */
...
/**
 * If any station is currently, selected, draw the bus station popup above it onto the canvas.
 *
 * @param p p5 instance.
 * @param mapXOffset Map container page X offset.
 * @param mapYOffset Map container page Y offset.
...
/**
 * This is the main p5.js sketch. `setup` is called once, `draw` is called each frame.
 * The framerate is set at the end of `setup` and is currently 24.
 */
const p5Sketch = (p: p5) => {
    p.setup = () => {
        ...
        // We do this here because p.color requires the current colorMode,
        // which we don't have outside this sketch.
